...
window.Game = (() => {
  ...
  function refreshParty(){
    ...
    avail.innerHTML=uniq.map(a=>`
      <div class="party-avail-row" style="display:flex;align-items:center;justify-content:space-between;padding:5px 0;border-bottom:1px solid #1e2a3a;gap:6px">
        <canvas class="party-avail-canvas" data-name="${a.name||a.n}" width="30" height="24" style="image-rendering:pixelated;background:rgba(0,0,0,0.3);border-radius:3px;flex-shrink:0"></canvas>
        <span style="flex:1;font-size:12px">${a.name||a.n}</span>
        <button class="party-add" data-n="${a.name||a.n}" style="padding:3px 8px;background:rgba(0,212,255,.15);border:1px solid #00d4ff;border-radius:5px;color:#00d4ff;cursor:pointer;font-size:11px">+ Add</button>
      </div>`).join('')||'<p style="font-size:12px;color:#555">Catch animals first!</p>';
    // Draw canvas sprites for party available list
    requestAnimationFrame(()=>{
      avail.querySelectorAll('.party-avail-canvas').forEach(cvs=>{
        const name=cvs.dataset.name;
        const c2=cvs.getContext('2d');
        c2.clearRect(0,0,30,24);
        try{ drawAnimalSprite(c2,name,0,0,30,animT,1); }catch(e){}
      });
    });
  ...
  function start(user,sd,saveCallback){
    ...
    function initMobileControls(){
      ...
      if(!mc) return;
      if(isMobile) mc.style.display='flex';

      // D-pad directional map
      ...
      // Action buttons
      const interactBtn=document.getElementById('mbtn-interact');
      const bagBtn=document.getElementById('mbtn-bag');
      const partyBtn=document.getElementById('mbtn-party');
      if(interactBtn){
        interactBtn.addEventListener('touchstart',e=>{e.preventDefault();checkInteract();Sound.sfx.click();},{passive:false});
        interactBtn.addEventListener('click',()=>checkInteract());
      }
      if(bagBtn){
        bagBtn.addEventListener('touchstart',e=>{e.preventDefault();togglePanel('inventory-panel');Sound.sfx.click();},{passive:false});
        bagBtn.addEventListener('click',()=>togglePanel('inventory-panel'));
      }
      if(partyBtn){
        partyBtn.addEventListener('touchstart',e=>{e.preventDefault();togglePanel('party-panel');Sound.sfx.click();},{passive:false});
        partyBtn.addEventListener('click',()=>togglePanel('party-panel'));
      }

      // Joystick-style: track touch on canvas for movement
      let touchId=null, touchStartX=0, touchStartY=0;
      canvas.addEventListener('touchstart',e=>{
        e.preventDefault();
        // Only if no existing touch tracked and panels closed
        if(touchId!==null) return;
        const inv=document.getElementById('inventory-panel');
        const party2=document.getElementById('party-panel');
        if(!inv.classList.contains('hidden')||!party2.classList.contains('hidden')) return;
      ...
      canvas.addEventListener('touchmove',e=>{
        ...
        if(touchId===null) return;
        let tch=null;
        for(let i=0;i<e.changedTouches.length;i++) if(e.changedTouches[i].identifier===touchId){tch=e.changedTouches[i];break;}
        if(!tch) return;
