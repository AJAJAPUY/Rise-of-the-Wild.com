...
window.Game = (() => {
  ...
  function start(user,sd,saveCallback){
    ...
    function getMobileBarH(){
      ...
      if(!mc||!mc.classList.contains('game-active')||getComputedStyle(mc).display==='none') return 0;
    ...
    wire('btn-quit-game',()=>{
      ...
      const mc=document.getElementById('mobile-controls');
      if(mc){ mc.style.display=''; mc.classList.remove('game-active'); }
