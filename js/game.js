...
window.Game = (() => {
  ...
  function applyNightTint(){
    ...
    if(isNight()){
      ...
      // Late day slight warm tint already gone — minimal
  ...
  function drawSky(){
    ...
    if(night>0.15){
      STARS.forEach(s=>{
        ...
        // Star color varies — blue-white, yellow, orange
    ...
    if(br>0.3&&b.id!=='ocean'&&b.id!=='volcanic'){
      ...
      // Crepuscular light tint at dawn/dusk
      ...
      clouds.forEach(cl=>{
        ...
        const cy2=cl.y*skyH;
        const r2=cl.r*br;
        // Fluffy cloud shape (multiple overlapping circles)
        ctx.beginPath();
        ctx.arc(cx2,cy2,r2,0,Math.PI*2);
        ctx.arc(cx2+r2*.9,cy2-r2*.3,r2*.75,0,Math.PI*2);
        ctx.arc(cx2+r2*1.8,cy2,r2*.85,0,Math.PI*2);
        ctx.arc(cx2+r2*.45,cy2-r2*.55,r2*.65,0,Math.PI*2);
        ctx.arc(cx2+r2*1.35,cy2-r2*.45,r2*.6,0,Math.PI*2);
        ctx.fill();
        // Shadow underside
        ctx.fillStyle=`rgba(${Math.round(cTint[0]*.82)},${Math.round(cTint[1]*.82)},${Math.round(cTint[2]*.8)},${cloudAlpha*.55})`;
        ctx.beginPath();ctx.ellipse(cx2+r2*.9,cy2+r2*.35,r2*1.6,r2*.28,0,0,Math.PI*2);ctx.fill();
        ctx.fillStyle=`rgba(${cTint[0]},${cTint[1]},${cTint[2]},${cloudAlpha})`;
      });
    }
  }
  ...
  function drawInn(sx,sy,br){
    ...
    ctx.fillStyle=blend('#666','#222',1-br*.8);ctx.fillRect(sx+58,sy-16,18,5);
    // Chimney stones
    ...
    // Smoke puffs
    for(let sm=0;sm<3;sm++){ctx.globalAlpha=br*(0.28-sm*.08);ctx.fillStyle=blend('#d0d0d0','#888',1-br*.5);ctx.beginPath();ctx.arc(sx+67+Math.sin(animT+sm)*3,sy-20-sm*12,4+sm*3,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=1;
    // Door with arch
  ...
  function drawAnimalSprite(c2,animalId,x,y,s,t,facing){
    ...
    if(id==='Wild Rabbit'||id==='rabbit'){
      ...
      // Tail
      c2.beginPath();c2.moveTo(x+s*.14,y+s*.5+bob);c2.quadraticCurveTo(x,y+s*.3+bob+Math.sin(t*2)*s*.05,x+s*.04,y+s*.4+bob);c2.stroke();
      ...
      // Wings spread
      const wingBeat=Math.sin(t*3+a?.bob||0)*s*.08;
      c2.fillStyle=fCol;
  ...
  function drawAnimals(){
    ...
    animals.forEach(a=>{
      ...
      // Shadow
      ctx.fillStyle=`rgba(0,0,0,${0.14*br+0.04})`;ctx.beginPath();ctx.ellipse(sx+s*.45,sy+s*.9,s*.3,s*.06,0,0,Math.PI*2);ctx.fill();
